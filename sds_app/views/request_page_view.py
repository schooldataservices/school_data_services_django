...
from sds_proj.settings import EMAIL_HOST_USER
import pytz
...
def get_prior_requests_context(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        html = render_to_string('sds_app/request_list.html', {'page_obj': page_obj}, request=request)
        return JsonResponse({
            'html': html.strip(),
            'total_results': paginator.count,
            'total_pages': paginator.num_pages,
    ...
    return render(request, 'sds_app/submit_request.html', context)
...
def create_request_config(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        html = render_to_string('sds_app/request_list.html', {'page_obj': pagination_context['page_obj']}, request=request)
        return JsonResponse({
            'html': html.strip(),
...
def historical_requests(request):
    ...
    return render(
        request,
        'sds_app/historical_requests.html',
        {
